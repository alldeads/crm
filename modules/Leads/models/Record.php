...
class Leads_Record_Model extends Vtiger_Record_Model {
    ...
    static function leadAttachments($record) {
        $db = PearDatabase::getInstance();
        $query = "SELECT DISTINCT vtiger_crmentity.crmid,vtiger_notes.title, vtiger_notes.folderid, vtiger_crmentity.smownerid, vtiger_crmentity.modifiedtime, vtiger_notes.filename, vtiger_notes.filelocationtype, vtiger_notes.filestatus FROM vtiger_notes inner join vtiger_senotesrel on vtiger_senotesrel.notesid= vtiger_notes.notesid left join vtiger_notescf ON vtiger_notescf.notesid= vtiger_notes.notesid inner join vtiger_crmentity on vtiger_crmentity.crmid= vtiger_notes.notesid and vtiger_crmentity.deleted=0 inner join vtiger_crmentity crm2 on crm2.crmid=vtiger_senotesrel.crmid LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid left join vtiger_seattachmentsrel on vtiger_seattachmentsrel.crmid =vtiger_notes.notesid left join vtiger_attachments on vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid left join vtiger_users on vtiger_crmentity.smownerid= vtiger_users.id  where crm2.crmid=".$record." AND vtiger_notes.filestatus = 1";
        $limitQuery = $query .' LIMIT 0,20';
        ...
        for($i=0; $i< $db->num_rows($result); $i++ ) {
            ...
            foreach($row as $col=>$val) {
                ...
                $relatedRecordList[$i]['fileid']   = $db->query_result($fileIdRes, 0, 'attachmentsid');
                $pathQuery = "SELECT path FROM vtiger_attachments WHERE attachmentsid=?";
                $path = $db->pquery($pathQuery, array($relatedRecordList[$i]['fileid']));
                $relatedRecordList[$i]['filepath'] = $db->query_result($path, 0, 'path');
                $relatedRecordList[$i]['filelocation'] = $relatedRecordList[$i]['filepath'] . '' . $relatedRecordList[$i]['filename'];
                $relatedRecordList[$i]['filename'] = $db->query_result($result, $i, 'filename');
            }
        }
