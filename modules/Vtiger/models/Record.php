...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	/**
	 * Function to get all the attachment files from specific record
     *@param <String> record
     *@return <Array> attachments
     */
    public static function getAttachments($record) {
        $db = PearDatabase::getInstance();
        ...
        $res = $db->pquery($sql, array($record));
        $count = $db->num_rows($res);
        $files = [];

        for($i=0;$i<$count;$i++)
        {
            $files['fileid'] = $db->query_result($res,$i,'attachmentsid');
            $files['filename'] = decode_html($db->query_result($res,$i,'name'));
            $files['filepath'] = $db->query_result($res,$i,'path');
        }
        return $count;
    }
}
