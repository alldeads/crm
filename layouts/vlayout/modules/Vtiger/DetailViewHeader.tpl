						...
						<div class="btn-toolbar">
                            {foreach item=DETAIL_VIEW_BASIC_LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}
								...
								<button class="btn" id="{$MODULE_NAME}_detailView_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($DETAIL_VIEW_BASIC_LINK->getLabel())}"
                                    ...
                                    {if $DETAIL_VIEW_BASIC_LINK->getLabel() eq "Generate PDF" }
										onclick="window.open('{$DETAIL_VIEW_BASIC_LINK->getUrl()}','mywin','left=20,top=20,width=600,height=650,toolbar=1,resizable=0');  return false;"
                                    {/if}
